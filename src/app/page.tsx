...
export default function Home() {
  const modules = [
    ...
    { name: 'Historial', href: '/movimientos', icon: 'SCROLL', color: 'bg-gray-700', desc: 'Movimientos y Anulaciones' },
  ];
    ...
    <main className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-4xl space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-lg text-gray-500">Sistema Administrativo Simple</p>
        </div>
          ...
          {modules.map((mod) => (
            ...
            >
              <div className={`h-14 w-14 rounded-xl ${mod.color} text-white flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                {mod.icon === 'SCROLL' ? <ScrollText className="w-7 h-7" /> : <mod.icon className="w-7 h-7" />}
              </div>
