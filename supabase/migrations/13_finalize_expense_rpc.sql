create or replace function public.finalize_expense(
  p_store_id     uuid,
...
begin
  ...
  values ('EXPENSE', p_total_amount, 'COMPLETED', p_entity_id, p_store_id)
  ...
  -- update stock atomically for items with a product_id
  update public.products prod
  set stock = prod.stock + (item->>'quantity')::numeric
  from jsonb_array_elements(p_items) item
  where (item->>'product_id') is not null
    and prod.id = (item->>'product_id')::bigint;

  select coalesce(sum((p->>'amount')::numeric), 0)
  into v_credit_total
  from jsonb_array_elements(p_payments) p
  where p->>'method' = 'CREDIT_PROVIDER';
